...
QWidget *MainWindow::createTab1()
{
    ...
    tablePreview = new QTableWidget;   // теперь поле класса
...
QWidget *MainWindow::createTab2()
{
    ...
    tableValid = new QTableWidget;   // поле класса
    ...
    tableBroken = new QTableWidget;   // поле класса
...
void MainWindow::onCreateFood()
{
    ...
    if (name.trimmed().isEmpty() || desc.trimmed().isEmpty() ||
        !okHunger || !okPrice || !okWeight ||
        hunger <= 0 || price <= 0.0 || weight <= 0.0) {
...
QList<Food> MainWindow::parseTxtFile(const QString &filePath, QStringList &errors)
{
    ...
    while (!stream.atEnd()) {
        ...
        if (name.isEmpty() || desc.isEmpty() || !okHunger || !okPrice || !okWeight ||
            hunger <= 0 || price <= 0.0 || weight <= 0.0) {
            errors << QString("Строка %1: неверные данные (проверьте формат и положительность)").arg(lineNum);
            continue;
        }
