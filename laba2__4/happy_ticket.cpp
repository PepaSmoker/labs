...
void happy_ticket::on_save_clicked()
{
    ...
    QString conditions;
    QString result;
    bool win = ui->win->isChecked();
    bool lose = ui->win->isChecked();

    QRegularExpression numberRegex("^\\d{1}\\-\\d{5}$");
    QRegularExpression dateRegex("^(0[1-9]|[12][0-9]|3[01])\\.(0[1-9]|1[012])\\.(19|20)\\d\\d$");
    QRegularExpression prizeRegex("^([A-ZА-ЯЁ][a-zа-яё]+)(\\s[A-ZА-ЯЁ][a-zа-яё]+)*$");
    QRegularExpression nameRegex("^([А-ЯЁ][а-яё]+)$");

    if (number.isEmpty()) {
        QMessageBox::warning(this, "Ошибка", "Заполните номер билета");
        return;
    }
    if (date.isEmpty())
    {
        QMessageBox::warning(this, "Ошибка", "Заполните дату");
        return;
    }
    if (name.isEmpty())
    {
    ...
    if(prize.isEmpty())
    {
        QMessageBox::warning(this, "Ошибка", "Заполните приз");
        return;
    }
    if(!win && !lose)
    {
        QMessageBox::warning(this, "Ошибка", "Заполните результат");
        return;
    }
    ...
    QMessageBox::information(this,"","Данные заполнены");
}